...
function sign (generator, data) {
  try {
    pkcs11.C_SignInit(generator.session, { mechanism: pkcs11js.CKM_SHA256_RSA_PKCS }, generator.keys.privateKey)
    pkcs11.C_SignUpdate(generator.session, Buffer.from(data))
    const signature = pkcs11.C_SignFinal(generator.session, Buffer.from(new ArrayBuffer(512)))
